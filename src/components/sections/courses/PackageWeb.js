    ...
    fetch(`http://localhost:5000/getcoursemappings/${packageId}`)
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          ...
          console.error(`Courses data for package ${packageId} is not an array`, data);
        }
      })
      .catch((error) => console.error(`Error fetching courses for package ${packageId}:`, error));
  };
          ...
          {packages.map((pkg) => (
            ...
            >
              ...
              <div className="mt-4">
                ...
                {/* Package Price */}
                <p className="text-lg font-bold mt-2">💰 ₹{pkg.package_price}</p>
              </div>
            </div>
          ))} 
        </div>
